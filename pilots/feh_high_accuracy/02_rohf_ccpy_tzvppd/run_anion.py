...
def find_checkpoint():
    ...
    if len(hits) != 1:
        raise RuntimeError(
            "Expected exactly one anion FeH- ROHF "
...
def load_rohf():
    ...
    if abs(
        ...
        raise RuntimeError(
            "Anion ROHF reference is not spin-pure quintet."
        )
...
def main():
    ...
    print(
        "ANION FeH- HIGH-LEVEL CONTROL SUMMARY"
    )
    print(
        "=" * 120
    )
